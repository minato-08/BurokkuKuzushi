...
**選択 UI のレイアウト方針**: 選択可能なスキルを **4 枚のカードとして横一列に共有表示** し、P1 / P2 が **それぞれ独立したカーソル** で選ぶ。両者のカーソルが同じカード群の上に乗るため、互いの選択がリアルタイムに見える。ローカル 2P の宿命だが、「相手が DOUBLE BALL を選んだ」情報はそれ自体がメタゲーム（対抗策を選ぶ動機になる）として機能する。同じカードを両者が選んでもよい。

- カードの並び順は実装の `SkillSelectUI.AllSkills` の index と一致させる（左→右で 0,1,2,3）。
- 各カードは「スキル名 + 短い説明 + アイコン（任意）」を静的に表示。**選択表現はカードの「色」で行う**（別カーソル GameObject は置かない）: 各カードに P1 用 / P2 用のハイライト Image を 1 枚ずつ重ね、選択中カードのみ点灯（P1=水色 / P2=赤）、未選択は透明、確定後は不透明に。`cardP1Highlights[4]` / `cardP2Highlights[4]` として `SkillSelectUI` にバインドする。
- 1P: A/D で左右移動・S 確定 / 2P: J/L で左右移動・K 確定。確定後は `$P{N}Status` を `READY!` に切替。両者確定で `BeginMatch()`。
...
#### コンボの定義
- **ボールがブロックに接触するたび**にカウントが 1 ずつ増える（破壊しなくても加算。Hard ブロックを削り切れずに弾いてもコンボは伸びる）。
  - 破壊数ではなく接触数で数えることで、「ボールをブロック面で躍らせ続ける」プレイそのものがコンボになる。Hard を相手にしても手応えが途切れない。
  - 1 回の衝突イベント（`OnCollisionEnter`）につき +1。Fire/Thunder/Pierce 等が 1 接触で複数ブロックを巻き込んでも、加算は接触回数ベース。
- 次の条件のいずれかで 0 にリセット:
  - **最後のブロック接触**から次のブロック接触まで **`comboTimeout`(=6.0s) 経過**
  - ボールが落下（DeadZone 通過）
  - ラウンド開始 / ラウンド終了

> **設計注意点**: タイマーは「パドル反射から」ではなく「最後のブロック接触から」で計測する。パドル反射起点にすると、序盤の低速球が急角度 (60° 超) で斜行した場合にブロック到達に時間がかかることがあり、コンボが意図せず切れる。ブロック接触起点ならコンボ維持のプレッシャーは「次のブロックを素早く叩く」に直結し、角度・速度への言い掛かりを排除できる。実装: `comboTimer[pi]` はブロック接触イベントで 0 にリセット（カウントアップ開始）し、`comboTimeout` を超えた瞬間にコンボを 0 にする。
