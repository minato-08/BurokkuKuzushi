...
- **Escape キー** または **P キー**でゲームをポーズ（一時停止）する。どちらのプレイヤーも押せる。
- ポーズ中は `Time.timeScale = 0`（IFreezable でフリーズするのではなく timeScale ゼロ）。ポーズは試合全体を止めるため、アリーナ分離は不要。
- 画面中央に `PAUSED — Press [P] or [Esc] to Resume` のオーバーレイを表示。
- 同じキーで再開（トグル）。
...
- ブロック・壁・パドルに衝突して反射する。
- パドル下のデッドゾーンに到達すると落下扱い（プレイヤー HP -`damageBallDrop`、デフォルト 5）。
...
- **初期位置**: アイマーは必ず **センター（真上 = 90°）** から始まる。`sin(0) = 0` なので最初のフレームで正面上方に向く。初めてプレイするプレイヤーが「とりあえず S を押せば上向きに飛ぶ」状態が自然に成立する。
- **自動発射**: `autoLaunchSec`（デフォルト 5s）経過後、その瞬間のアイマー角度で自動発射する（最良角度を待つような「インテリジェント」な自動発射はしない）。自動発射時は `LAUNCHED!` ラベルを 0.5s 表示。
- **ブロック接近時の自動発射短縮**: 最下段ブロックが `blockDeadZoneY + 3` 以内に入ると、残り `autoLaunchSec` を `minAutoLaunchSec`（デフォルト 1.5s）に短縮する（危機察知して急いで発射する演出）。
...
発表（Phase F-Playtest）では即興感を高めるため、ラウンド開始時に BlockSpawner が `StartRound()` 内で 2 行をプリスポーンする（通常行構成）。これにより `GO!` 直後から狙うべきブロックが視認でき、初プレイヤーが LaunchAimer を使う理由がすぐわかる。デモモード限定（`demoMode = true` SerializeField でトグル）。通常の対戦では空スタートを維持する。
...
- 最下段ブロックが `blockDeadZoneY + 1.5` 以内に入ると、`P1BlockDeadLine`（UIアリーナ下端のライン）が赤く点滅開始（0.4s 周期）。
- `blockDeadZoneY + 0.5` 以内になると点滅が高速化（0.15s 周期）。
...
|---|---|---|
| 全 Absorb 行 | Absorb × 6 | ボールが全部減速。コンボ継続困難になる緊急課題 |
| 全 Explosive 行 | Explosive × 6 | 1 発で連鎖爆発。リスクとリターンが大きい |
| スパース行 | 4 ブロック + 隙間 2 個（ランダム配置） | 隙間を通せる角度を狙う技術的な挑戦 |
| ラッシュ行 | 通常 × 6（ブロック隙間 0）+ 1s 後即もう 1 行 | 一時的に 2 行同時に降下する圧力 |
...
| `spawnIntervalDecayPerMin` | 0.5s | 1 分ごとにスポーン間隔が縮まる量 |
| `spawnIntervalMin` | 2.5s | スポーン間隔の下限 |
| `descentSpeedBase` | 0.3 u/s | ラウンド開始時の降下速度 |
| `descentSpeedGainPerMin` | 0.05 u/s | 1 分ごとに降下速度が増える量 |
| `descentSpeedMax` | 0.6 u/s | 降下速度の上限 |

#### 時間ごとの圧力感

| ラウンド経過時間 | スポーン間隔 | 降下速度 | 体感 |
|---|---|---|---|
| 0:00 | 5.0 s | 0.30 u/s | 穏やか・助走 |
| 1:00 | 4.5 s | 0.35 u/s | じわじわ増す |
| 2:00 | 4.0 s | 0.40 u/s | 本番テンション |
| 3:00 | 3.5 s | 0.45 u/s | 息が詰まる |
| 4:00 | 3.0 s | 0.50 u/s | 常に行が降る |
| 5:00+ | 2.5 s（下限） | 0.55〜0.60 u/s | 限界ゾーン |
...
**設計意図**: 発表（5分程度の 1 ラウンド）が「ぬるすぎる序盤 → 終盤でパニック」ではなく「全体通じて面白い」体験を保証する。3 分でほぼ満圧になるため、ほとんどの試合で「限界ゾーンに到達するかどうか」を気にする必要はない。
...
| CATCH & SHOOT | ゲージ満タン → 攻撃アイテム確定の强ルーティング | ゲージ使用後の隙。操作が複雑 | 相手に「攻撃アイテム来る」と構える時間を与える |
...
**ローカル 2P の情報透明性について**: このゲームは同一画面ローカル 2P のため、両プレイヤーが互いの HUD を常に見られる。一般に「隠し情報がない」とゲームが単純化するリスクがあるが、このゲームは逆に活用している。

- **相手の RETALIATION READY が見える**: 攻撃側は「今送ると 2 倍返しを受ける」と知って判断できる。「あえて踏む」「踏まないように待つ」の戦略的読み合いが成立する。
- **相手のコンボ・Ball Heat が見える**: 「相手がもうすぐマイルストーンを達成する」がわかり、妨害タイミングを急ぐ判断が生まれる。
- **相手のスキルゲージが見える**: 「あいつそろそろゲージ満タンだから DOUBLE BALL 来るかも」という先読みが可能。

これは「隠すと説明が難しくなる」情報を「見せることで戦略レイヤーを追加する」設計。ローカル 2P ゲームとして正直に機能させる。
...
| MVP ラベル（任意） | コンボ / スコア / 攻撃数のうち 1 項目で最高値を出した側に小さな `⭐ MVP` 表示 |
...
- AI 相手は **EASY 固定**（チュートリアル中のみ、難易度選択不可）。AI は通常時は妨害を仕掛けてこない（受動学習に集中）。**Step 7 のみ**チュートリアル進行マネージャが AI 側に Harden 妨害を 1 回スクリプト発火させる（自然な攻撃アイテム取得を待たず即座に SendInterference を呼ぶ）。これにより RetaliationWindow を必ず開かせ、Step 7 のクリア条件を満たせるようにする。
- **スキップ可否**:
...
発表で 1 人プレイができるよう、簡易 AI を P2 に組み込む選択肢を持たせる（Phase G+ で実装、可能なら）。「アクセシビリティ的に 1 人でも遊べる」価値を優先（ローカル 2P は同席必須なので）。

#### AI のアーキテクチャ概要

`AIPlayerController` は `PlayerController` を継承（または同等インターフェース）し、人間入力の代わりに以下のロジックでパドルを動かす：

```
毎フレーム:
  1. 視界更新 — 現在のボール位置 / 速度 / アクティブアイテム位置を取得
  2. 目標 X 算出 — ボール落下軌道予測 or 最近アイテムの追跡
  3. 移動指示 — 目標 X へパドルを `aiPaddleSpeed` で近づける（人間より遅い設定）
  4. 発射判断 — LaunchAimer 起動中なら `aiAimAccuracy` の確率で「真上 ±N°」で発射
  5. スキル発動 — Energy 満タンかつ条件成立時に Q を押す
```

予測軌道は単純化のため「次の壁反射まで」のみ計算（再帰せず1階層）。AI が攻撃アイテムを取りに行くかは `aiAttackItemBias` で決定。

#### 難易度別パラメータ

| パラメータ | EASY | NORMAL | HARD | 備考 |
|---|---|---|---|---|
| `aiPaddleSpeed` | 10 u/s | 14 u/s | 18 u/s | 人間 P2 の上限 16 u/s より HARD は若干速い |
| `aiReactionDelaySec` | 0.30s | 0.15s | 0.05s | ボール速度変化や妨害着弾を認識するまでの遅延 |
| `aiAimAccuracy` | 0.40 | 0.70 | 0.90 | 真上 ±15° を狙う成功率（外れは ±60° のランダム角度）|
| `aiAttackItemBias` | 0.30 | 0.70 | 1.00 | 攻撃アイテムを能動的に追う確率（残りは無視 = 通り過ぎる）|
| `aiSkillUseDelay` | 2.0s | 0.8s | 0.2s | エナジー満タン到達から発動までの待機（人間らしさ）|
| `aiPredictDepth` | 0 | 1 | 2 | 軌道予測の反射回数（0 = 現在位置のみ追跡）|
| `aiLanePreference` | センター固定 | やや追跡 | 完全追跡 | LaunchAimer 発射前のパドル位置選好 |

#### 設計意図

- **EASY は明確に弱い**: パドル速度が遅く、攻撃を取りに来ない → 初心者でも勝てる
- **NORMAL は接戦狙い**: HP 帯バンドのカムバック性が機能する程度の力差
- **HARD は本気**: 速度・精度ともに人間上級者と互角。アイテム判断は完璧（妨害を遠慮しない）

#### 制約

- AI は HitStop / Pause / TrapBall_Reversed 等の状態変化を正しく扱う必要がある（自分のターンで操作不能）
- スキル選択は AI 専用ロジック（HARD なら相手スキルに対するメタ選択、NORMAL/EASY はランダム）
- AI の「思考」は `unscaledTime` ではなく `Time.time`（ポーズに従う）で動作させる
...
- 1P / 2P それぞれ 4 種類のアクション（左移動・右移動・発射・スキル）をリマップ可能
- Pause / メニュー操作は固定（Esc / Enter / Space）
- ゲームパッド対応（Xbox / PS / 任意の HID）— `Unity Input System` 経由
- 競合検知（同じキーを 2 つのアクションに割当てる場合は警告）
- PlayerPrefs キー: `keybind.p1.left` / `keybind.p1.right` / `keybind.p1.launch` / `keybind.p1.skill` （P2 同様）

### 13.6 言語

- 発表時点では日本語のみ（テキストはハードコード）。
- 将来的な多言語化（i18n）に備え、`LocalizationTable.csv` 形式の string ID 化を Phase G+ で検討。ID 化の対象: HUD 固定テキスト、メニュー項目、エッジケースメッセージ、SE 字幕。
- 試合中の自動切替（プレイヤーが地域別のローカルキーボードを使う場合）は対象外。設定画面で明示的に切替する。
