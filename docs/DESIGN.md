...
- 次の条件のいずれかで 0 にリセット:
  - **最後のブロック破壊**から次のブロック破壊まで **`comboTimeout`(=6.0s) 経過**
  - ボールが落下（DeadZone 通過）
  - ラウンド開始 / ラウンド終了

> **設計注意点**: タイマーは「パドル反射から」ではなく「最後のブロック破壊から」で計測する。パドル反射起点にすると、序盤の低速球が急角度 (60° 超) で斜行した場合にブロック到達に時間がかかることがあり、コンボが意図せず切れる。ブロック破壊起点ならコンボ維持のプレッシャーは「次のブロックを素早く叩く」に直結し、角度・速度への言い掛かりを排除できる。実装: `comboTimer[pi]` はブロック破壊イベントで 0 にリセット（カウントアップ開始）し、`comboTimeout` を超えた瞬間にコンボを 0 にする。
