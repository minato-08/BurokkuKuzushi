...
public class SkillSelectUI : MonoBehaviour
{
    ...
    private void HandleInput()
    {
        ...
        if (!p1Confirmed)
        {
            if (kb.aKey.wasPressedThisFrame) { p1Index = Wrap(p1Index - 1); AudioManager.Instance?.PlayUiMove(); }
            if (kb.dKey.wasPressedThisFrame) { p1Index = Wrap(p1Index + 1); AudioManager.Instance?.PlayUiMove(); }
            if (kb.sKey.wasPressedThisFrame) { p1Confirmed = true; AudioManager.Instance?.PlayUiConfirm(); }
        }
        if (!p2Confirmed)
        {
            if (kb.jKey.wasPressedThisFrame) { p2Index = Wrap(p2Index - 1); AudioManager.Instance?.PlayUiMove(); }
            if (kb.lKey.wasPressedThisFrame) { p2Index = Wrap(p2Index + 1); AudioManager.Instance?.PlayUiMove(); }
            if (kb.kKey.wasPressedThisFrame) { p2Confirmed = true; AudioManager.Instance?.PlayUiConfirm(); }
        }
    }
